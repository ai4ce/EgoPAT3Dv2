...
import cv2
from data_utils.testDataLoader import testDataLoader_without_image as testDataLoader
import datetime
import logging
...
def main(args):
    ...
    for batch_id, data in tqdm(enumerate(finaltestDataLoader, 0), total=len(finaltestDataLoader), smoothing=0.9):
            ...
            center = torch.FloatTensor(args.num_point,3).zero_()
            ...
            with open(result_path, 'w') as f:
                ...
                for xx in pred:
                    ...
                    center[:,0] = float(dcon(xx[0][0]))
                    center[:,1] = float(dcon(xx[0][1]))
                    center[:,2] = float(dcon(xx[0][2]))
                    # print(points[count,:,:].transpose(-2,-1).shape)
                    pos = pos = np.argmin(np.linalg.norm(center-points[count,:,:],axis=1))
                    data = str(float(points[count,pos,0]))+','+str(float(points[count,pos,1]))+','+str(float(points[count,pos,2]))
